<?php
return [
    'crawler' => 'Crawler',
    'crawler_indexing' => 'Indexierung',
    'crawler_analytics' => 'Analytics',
    'crawler_analytics_queries' => 'Suchanfragen',
        
    //builderindex
    'crawler_index' => 'Index',
    'builderindex_url' => 'Url',
    'builderindex_title' => 'Bezeichnung',
    'builderindex_language_info' => 'Sprachinfo',
    'builderindex_content' => 'Inhalt',
    'builderindex_url_found' => 'URL auf folgender Seite gefunden',

    //index
    'crawler_builderindex' => 'Zwischenspeicher',
    'index_url' => 'Url',
    'index_title' => 'Bezeichnung',
    'index_language_info' => 'Sprachinfo',
    'index_content' => 'Inhalt',
    'index_url_found' => 'URL auf folgender Seite gefunden',

    // 1.0.6

    'added_to_index' => 'Hinzugefügt',
    'last_update' => 'Zuletzt aktualisiert',
    'dashboard_title' => 'Neuste Suchanfragen ohne Resultat',

    // 2.0

    'link_url' => 'Link-Ziel',
    'link_url_found_on_page' => 'Quelle',
    'link_title' => 'Link-Text',
    'link_is_ignored' => 'Ist ignoriert',
    'link_response_status' => 'Response Status',
    'link_created_at' => 'Created at',
    'link_updated_at' => 'Updated at',
    'done_button_label' => 'Gelöst',
    'done_button_success' => 'Link has been removed from the List.',
    'crawler_menu_links' => 'Links',
   'crawler_menu_stats' => 'Statistik',
   'stats_from' => 'Von',
   'stats_to' => 'Bis',
   'stats_searches_title' => 'Suchanfragen',
   'stats_no_results_title' => 'Anfragen ohne Fundstellen',
   'stats_most_suggestions_title' => 'Häufigste Suchbegriff-Vorschläge',
   'stats_no_data' => 'Keine Daten verfügbar',
   'legend_total' => 'Total',
   'legend_results' => 'Ohne Fundstellen',
   'legend_no_results' => 'Mit Fundstellen',
];
